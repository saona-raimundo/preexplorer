...
fn increasing_comparisons() {
    ...
    let plotting_1 = pre::Sequence::new(data_1).set_title("First").to_owned();

    // Another sequence

    let data_2 = vec![0., 1.4, 10., 4.];
    let group_of_plottings = vec![pre::Sequence::new(data_2).set_title("Second").to_owned()];
    ...
    let group_of_plottings = vec![pre::Sequence::new(data_3).set_title("Third").to_owned()];
